...
>(({ className, ...props }, ref) => {
  return (
    <div
      ref={ref}
      className={cx(
        "relative flex items-center rounded-lg bg-card px-1 shadow-lg shadow-black/30 ring-1 ring-border",
        className,
      )}
      {...props}
    />
...
const CommandBarCommand = React.forwardRef<HTMLButtonElement, CommandProps>(
  (
    ...
    return (
      <span
        className={cx(
          "flex items-center gap-x-2 rounded-lg bg-card p-1 text-base font-medium text-foreground outline-none transition focus:z-10 sm:text-sm",
      ...
      >
        <button
          ...
          className={cx(
            // base
            "flex items-center gap-x-2 rounded-md px-1 py-1 hover:bg-accent",
            // focus
            "focus-visible:bg-accent focus-visible:hover:bg-accent",
