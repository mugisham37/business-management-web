...
export class MfaService {
  ...
  constructor(
    ...
    this.appName = this.configService.get<string>('MFA_APP_NAME', 'Business Management Platform');
    this.issuer = this.configService.get<string>('MFA_ISSUER', 'Business Management Platform');
  ...
  async generateMfaSetup(userId: string, tenantId: string, userEmail: string): Promise<MfaSetupResponse> {
    ...
    try {
      ...
      this.logger.log(`MFA setup generated for user`, { userId, tenantId });
  ...
  async enableMfa(userId: string, tenantId: string, token: string): Promise<void> {
    ...
    try {
      ...
      // Publish auth event
      await this.authEventsService.publishMfaEnabled(userId, tenantId);
  ...
  async disableMfa(userId: string, tenantId: string, token?: string, backupCode?: string): Promise<void> {
    ...
    try {
      ...
      // Publish auth event
      await this.authEventsService.publishMfaDisabled(userId, tenantId, token ? 'totp' : 'backup_code');
  ...
  async verifyMfaToken(userId: string, tenantId: string, token: string): Promise<MfaVerificationResult> {
    ...
    try {
      ...
      if (isBackupCode) {
        ...
        this.logger.log(`MFA backup code used`, { userId, tenantId, remainingCodes: updatedBackupCodes.length });
  ...
  async generateNewBackupCodes(userId: string, tenantId: string, token: string): Promise<string[]> {
    ...
    try {
      ...
      this.logger.log(`MFA backup codes regenerated`, { userId, tenantId });
